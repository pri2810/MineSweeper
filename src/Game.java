...
public class Game
{
	...
	private int undo;
	private int Bombs;
	private char[][] dummyGrid;     //stores present moves according to input
	int Moves = 0;
	...
	{													//for user inside game constructor
		...
		previous = new char[1000000][Rows][Columns];
		for(int i = 1; i < r + 1; i++)
		{
			for(int j = 1; j < c + 1; j++)
			{
				...
				{								           //each non'*' (bomb) cell
					for(int k = i - 1; k < i + 2; k++)				
						for(int m = j - 1; m < j + 2; m++)						
							if(arr[k][m] == '*')
								count++;		          // number of bombs surrounded by arr[i][j]
		...
		for(int i = 1; i < r + 1; i++)              // making actualGrid[m][n] from arr[m+2][n+2] and 
			{								        //initializing dummyGrid with all cells of it 
	...
	public boolean[] check(String input,String previous_input)
	{
		...
		else               //input is x y flag/check
		{
			...
			if((x < 0 || x >= Rows) || (y < 0 || y >= Columns) || (f != 1 && f != 0))
			{
			...
			for(int i = 0; i < Rows; i++)
				for(int j = 0; j < Columns; j++)		
					if(i == x && y == j)
					{
						if(f == 1 && (dummyGrid[i][j] == '-' || dummyGrid[i][j] == '|'))  //checking is possible for already flagged ones
							{		
						...
						else if(f == 0 && dummyGrid[i][j] == '-')     //flagging is only possible for non checked
							{   
								dummyGrid[i][j] = '|';
								if(actualGrid[i][j] == '*')
										Bombs --;
